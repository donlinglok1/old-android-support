...
public class TypedValues {
	...
	/**
	 * Additional information about where the value came from; only set for
	 * strings.
	 */
	...
	private static final float MANTISSA_MULT = 1.0f / (1 << TypedValues.COMPLEX_MANTISSA_SHIFT);
	 ...
	 */
	public static final float complexToFloat(final int complex) {
		return (complex & TypedValues.COMPLEX_MANTISSA_MASK << TypedValues.COMPLEX_MANTISSA_SHIFT)
				* RADIX_MULTS[complex >> TypedValues.COMPLEX_RADIX_SHIFT & TypedValues.COMPLEX_RADIX_MASK];
	}
	 ...
	 * @param metrics
	 *            Current display metrics to use in the conversion -- supplies
	 *            display density and scaling information.
	 *
	 * @return The complex floating point value multiplied by the appropriate
	 *         metrics depending on its unit.
	 */
	public static final float complexToDimension(final int data, final DisplayMetrics metrics) {
		return applyDimension(data >> COMPLEX_UNIT_SHIFT & COMPLEX_UNIT_MASK, complexToFloat(data), metrics);
	}

	/**
	 * Converts a complex data value holding a dimension to its final value as
	 * an integer pixel offset. This is the same as {@link #complexToDimension},
	 * except the raw floating point value is truncated to an integer (pixel)
	 * value. The given <var>data</var> must be structured as a
	 * {@link #TYPE_DIMENSION}.
	 *
	 * @param data
	 *            A complex data value holding a unit, magnitude, and mantissa.
	 ...
	 */
	public static final int complexToDimensionPixelOffset(final int data, final DisplayMetrics metrics) {
		return (int) applyDimension(data >> COMPLEX_UNIT_SHIFT & COMPLEX_UNIT_MASK, complexToFloat(data), metrics);
	}

	/**
	 * Converts a complex data value holding a dimension to its final value as
	 * an integer pixel size. This is the same as {@link #complexToDimension},
	 * except the raw floating point value is converted to an integer (pixel)
	 * value for use as a size. A size conversion involves rounding the base
	 * value, and ensuring that a non-zero base value is at least one pixel in
	 * size. The given <var>data</var> must be structured as a
	 * {@link #TYPE_DIMENSION}.
	 *
	 * @param data
	 *            A complex data value holding a unit, magnitude, and mantissa.
	 ...
	 *         multiplier and units.
	 */
	public static final int complexToDimensionPixelSize(final int data, final DisplayMetrics metrics) {
	...
	public static final float complexToDimensionNoisy(final int data, final DisplayMetrics metrics) {
		final float res = complexToDimension(data, metrics);
		System.out.println("Dimension (0x"
				+ (data >> TypedValues.COMPLEX_MANTISSA_SHIFT & TypedValues.COMPLEX_MANTISSA_MASK) + "*"
				+ RADIX_MULTS[data >> TypedValues.COMPLEX_RADIX_SHIFT & TypedValues.COMPLEX_RADIX_MASK] / MANTISSA_MULT
				+ ")" + DIMENSION_UNIT_STRS[data >> COMPLEX_UNIT_SHIFT & COMPLEX_UNIT_MASK] + " = " + res);
		return res;
	}

	/**
	 * Converts an unpacked complex data value holding a dimension to its final
	 * floating point value. The two parameters <var>unit</var> and
	 * <var>value</var> are as in {@link #TYPE_DIMENSION}.
	 *
	 * @param unit
	 ...
	 *         metrics depending on its unit.
	 */
	public static final float applyDimension(final int unit, final float value, final DisplayMetrics metrics) {
		switch (unit) {
	...
	/**
	 * Return the data for this value as a dimension. Only use for values whose
	 * type is {@link #TYPE_DIMENSION}.
	 *
	 * @param metrics
	 *            Current display metrics to use in the conversion -- supplies
	 *            display density and scaling information.
	 ...
	 *            A complex data value holding a unit, magnitude, and mantissa.
	 * @param base
	 *            The base value of this frKey. In other words, a standard
	 *            fraction is multiplied by this value.
	 * @param pbase
	 *            The parent base value of this frKey. In other words, a parent
	 *            fraction (nn%p) is multiplied by this value.
	 *
	 * @return The complex floating point value multiplied by the appropriate
	 *         base value depending on its unit.
	 */
	public static final float complexToFraction(final int data, final float base, final float pbase) {
	...
	/**
	 * Return the data for this value as a frKey. Only use for values whose type
	 * is {@link #TYPE_FRACTION}.
	 *
	 * @param base
	 *            The base value of this frKey. In other words, a standard
	 *            fraction is multiplied by this value.
	 ...
	 * value. For example, a color type will be converted to a string of the
	 * form #aarrggbb.
	 *
