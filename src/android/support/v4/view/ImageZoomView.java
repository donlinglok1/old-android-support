...
public class ImageZoomView extends RelativeLayout {
    ...
    private void setViewImage(final View view) {
	...
	if (originalImageWidth == frameWidth || originalImageHeight == frameHeight) {
	    ...
	    // center horizontally
	    minZoom = Math.max(frameWidth / originalImageWidth, frameHeight / originalImageHeight);
	...
	private void doDrag(final View view, final MotionEvent event) {
	...
	private void doPointerUp() {
	    mode = NONE;
	}

	private void doPointerDown(final MotionEvent event) {
	...
	private void doActionDown(final MotionEvent event) {
	    lastX = (int) event.getRawX();
	    lastY = (int) event.getRawY();
    ...
    private void adjustPan(final View view) {
	matrix.getValues(matrixValues);
