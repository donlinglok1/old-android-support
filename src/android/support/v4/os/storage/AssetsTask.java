...
public class AssetsTask extends AsyncTask<Void, Void, String> {
    ...
    private void copyAssets(final Context context, final String folderName) throws IOException {
	final String dirPathString = NString.parse(Files.getTempFolderPath(context), folderName);
	...
	if (null != files) {
		...
		try {
			...
			if (file.contains(".")) {
			    ...
			    if (!outFile.exists()) {
				final InputStream inputStream = assetManager.open(NString.parse(folderName, file));
			    ...
			    copyAssets(context, NString.parse(file, Strings.SLASH));
			}
		    }
